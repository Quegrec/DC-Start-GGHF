      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="relative">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
            ...
            <div className="text-2xl font-bold text-[#00D1FF]">{topTrait.trait}</div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            {personalityData.map((item) => {
              const isTop = item.trait === topTrait.trait || item.trait === secondTrait.trait;
              
              return (
                <div key={item.trait}>
                  <div className="flex items-center justify-between mb-2">
                    <span className={`font-medium ${isTop ? 'text-[#00D1FF]' : 'text-white/80'}`}>
                      {item.trait}
                    </span>
                    <span className={`font-semibold ${isTop ? 'text-[#00D1FF]' : 'text-white/60'}`}>
                      {item.value}%
                    </span>
                  </div>
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-[#00D1FF]/10 to-[#00D1FF]/5 border border-[#00D1FF]/30 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-[#00D1FF]">Personality Insight</h3>
              </div>
              
              <p className="text-sm text-white/80 leading-relaxed">
                Your <span className="font-semibold text-[#00D1FF]">{topTrait.trait.toLowerCase()}</span> score 
                ({topTrait.value}%) aligns you with the <span className="font-semibold text-[#00D1FF]">Sniper</span> path, 
                while your high <span className="font-semibold text-[#00D1FF]">{secondTrait.trait.toLowerCase()}</span> 
                ({secondTrait.value}%) makes you a natural <span className="font-semibold text-[#00D1FF]">Medic</span>. 
                This unique combination suggests you excel at precision teamwork and supportive strategies.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="text-2xl font-bold text-[#00D1FF] mb-1">{topTrait.value}%</div>
              <div className="text-xs text-white/60">Highest Trait</div>
            </div>
            <div className="text-center p-4 rounded-xl bg-white/5 border border-white/10">
              <div className="text-2xl font-bold text-[#00D1FF] mb-1">
                {Math.round(personalityData.reduce((sum, item) => sum + item.value, 0) / personalityData.length)}%
              </div>
